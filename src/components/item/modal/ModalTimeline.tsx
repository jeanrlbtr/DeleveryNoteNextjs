      ...
      <div className='max-h-[60vh] flex '>
        <div className=' flex flex-col md:grid md:grid-cols-9 mx-auto p-2 '>
          {dataItems.map((progress: any, index: number) => {
            const date = new Date(progress.timestamp * 1000).toDateString();
            const time = new Date(progress.timestamp * 1000).toLocaleTimeString();
                    ...
                    <div className={`w-full col-start-1 col-end-5 p-4 rounded-xl my-4 ml-auto border border-gray-300`}>
                      <h3
                        className={`font-semibold ${
                          progress.status === 'FINISH' ? 'text-[green]' : 'text-[#ff9100]'
                        } text-[14px] md:text-[17px] mb-1 `}
                      >
                        {progress.status} <span>({progress.user.name})</span>
                      </h3>
                      <p className='text-gray-600 w-[300px] text-[12px] md:text-[17px]'>{progress.note}</p>
                    </div>
