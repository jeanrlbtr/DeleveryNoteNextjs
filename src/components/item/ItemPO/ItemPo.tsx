...
const status = ['ALL', 'PROCESS', 'CANCELED', 'UNCOMPLETED', 'FINISH', 'MODIFIED'];
...
const ListItemPo = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryFn: async () => {
      const url =
        statusQuery && statusQuery !== 'ALL' ? `/delivery/v1/note/items?status=${statusQuery}&page=${page}` : `/delivery/v1/note/items?page=${page}`;
            ...
            {noPo && (
              <div className='w-full h-max p-2 rounded-[5px] bg-white mt-[30px]'>
                <div className={`flex justify-between items-center  ${roboto.className}`}>
                  ...
                  <p className={`text-[14px] font-[500]  ${noPo.status !== 'FINISH' ? 'text-[#b88c3b]' : 'text-[green]'}`}>{noPo.status}</p>
                </div>
